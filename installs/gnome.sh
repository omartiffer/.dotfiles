...
[[ -f "$EXT_LIST" ]] || {
  log ERR "Missing extension list: $EXT_LIST"
  exit 1
}
[[ -f "$EXT_DIS" ]] || {
  log ERR "Missing disable list: $EXT_DIS"
  exit 1
}
...
main() {
  ...
  if [[ "$dry_run" == "false" ]]; then
    if wget -q -O "$DOTFILES/gnome-files/gnome-shell-extension-installer" "$EXT_INSTALLER_REPO" 2>/dev/null; then
      log PLAIN "Download successful, installing..."
      install -m 755 "$DOTFILES/gnome-files/gnome-shell-extension-installer" "$HOME/.local/my-scripts/"
    else
